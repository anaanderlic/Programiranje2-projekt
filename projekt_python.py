    ...
    for leto in leta:
        for disciplina in discipline:
            ...
            if "PAGE NOT FOUND" not in vsebina:
                ...
                s2 = vsebina.find_all('span', class_='styles__CountryName-sc-1r5phm6-1 ihDbcl')
                drzave = []
                for drzava in s2:
                    drzava = re.search(r'<span class="styles__CountryName-sc-1r5phm6-1 ihDbcl" data-cy=".*?">(?P<drzava>\D{3})</span>',str(drzava))
                    drzava = drzava.group('drzava')
                    drzave.append(drzava)

                #pridobitev vseh imen tekmovalcev
                s3 = vsebina.find_all('h3', class_="styles__AthleteName-sc-1yhe77y-3 edUsrV")
                imena = []
                for ime in s3:
                    ime = re.search(r'<h3 class="styles__AthleteName-sc-1yhe77y-3 edUsrV" data-cy="athlete-name">(?P<ime>.*?)</h3>',str(ime))
                    ime = ime.group('ime')
                ...
                s4 = vsebina.find_all('span', class_='styles__Info-sc-cjoz4h-0 eVxhcK')
                rezultati = []
                for rezultat in s4:
...
def rojstni_dnevi():
    ...
    with open('tekmovalci.txt', 'r', encoding='utf8') as file:
        for tekmovalec in file:
            ...
            s = vsebina.find_all('div', class_='col-right')
            if s == []:
                continue
            leto = re.search(r'<div class="col-right">(?P<bd>\d{4})</div>',str(s[-1]))
            ...
            kr = vsebina.find('span', class_='mobile-text')
            if kr == '':
                continue
            kratica = re.search(r'<span class="mobile-text">(?P<kratica>.*?)</span>',str(kr))
            if kratica == None:
                continue
            kratica = kratica.group('kratica')
            drzave['kratica'] = kratica 

            dr = vsebina.find('span', class_='desktop-text d-none')
            if dr == '':
                continue
            drzava = re.search(r'<span class="desktop-text d-none">(?P<drzava>.*?)</span>',str(dr))
            if drzava == None:
                continue
            drzava = drzava.group('drzava')
...
def narisi_zemlevid(slovar_kolicin, slovar_koordinat):
    ...
    plt.close()
    fig.savefig('grafi/od kod prihajajo atleti.pdf')
